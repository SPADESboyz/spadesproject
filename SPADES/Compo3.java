...
public class Compo3 extends MouseInputAdapter {
private JPanel p1;
private JLabel l1;
private JLabel[] la1;
private Cards[] ca1;
...
public Compo3(JPanel p1, JLabel l1, Cards[] ca1, JLabel[] la1){
this.p1 = p1;
this.l1 = l1;
this.ca1 = ca1;
this.la1 = la1;
...
/** So the below code is to remove the card that is pressed or rather selected from the panel */ 
this.p1.remove(this.l1);
this.p1.remove(this.l1);
this.p1.validate();
this.p1.repaint();
JOptionPane.showMessageDialog(null, "YOU HAVE SELECTED A CARD!");
JOptionPane.showMessageDialog(null, "NOTE"+" "+SetPlayers.names4().toUpperCase()+" "+"THAT YOU CAN NOT SELECT ANOTHER CARD UNTIL ALL PLAYERS HAVE PLAYED!");

/** The below method is to find the index number of the selected card */
for(int i = 0 ; i < this.la1.length ; i++){
if(this.la1[i]== this.l1){
this.indexNumber = i;
}
}
...
JOptionPane.showMessageDialog(null, "You have selected"+" "+ca1[this.indexNumber].printCards());

//---------------------
try{
Cards cards = new Cards("Spades","Ace",27,ImageIO.read(new File("Ace_Of_Spades.png")));

Cards[] playerA =  Spades.play1();
Cards[] playerB = Spades.play2();    
Cards[] playerC = Spades.play3();
Cards[] playerD =  Spades.play4();
...
//==================
LinkedList<Cards> Table=createtable(Compo.CardValue(),Compo1.CardValue(),Compo2.CardValue(),this.indexNumber,PLayer1,PLayer2,PLayer3,PLayer4);
...
if(KeepTrack==true){
JOptionPane.showMessageDialog(null,"LOOKS LIKE"+" "+ SetPlayers.names1().toUpperCase()+" "+"WON THIS ROUND!");
scores[0] =  p1++;
break;
}
KeepTrack=Findplayer(PLayer2,returncard);
if(KeepTrack==true){
JOptionPane.showMessageDialog(null,"LOOKS LIKE"+" "+SetPlayers.names2().toUpperCase()+" "+"WON THIS ROUND!");
scores[0] =  p2++;
break;
}
KeepTrack=Findplayer(PLayer3,returncard);
if(KeepTrack==true){
JOptionPane.showMessageDialog(null,"LOOKS LIKE"+" "+SetPlayers.names3().toUpperCase()+" " +"WON THIS ROUND!");
scores[0] =  p3++;
break;
}
KeepTrack=Findplayer(PLayer4,returncard);
if(KeepTrack==true){
JOptionPane.showMessageDialog(null,"LOOKS LIKE"+" "+SetPlayers.names4().toUpperCase()+" "+"WON THIS ROUND! ");
scores[0] =  p4++;
break;
}
}
...
JOptionPane.showMessageDialog(null,"ERROR DETECTED! ");
 ...
 JOptionPane.showMessageDialog(null, "PLEASE"+" "+SetPlayers.names1().toUpperCase()+" "+"NOW SELECT YOUR CARD TO START NEXT ROUND!" );
}
public int CardValue(){
return ca1[this.indexNumber].printValue();
}
///----------------------------------------------------------------
...
for(int j=0;j<len2;j++){// has the leading suit and the lower value
...
for(int k=0;k<len2;k++){// does not have the suit but has a low value card
Cards pl=player.get(k);
